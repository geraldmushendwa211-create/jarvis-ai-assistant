...
| `scripts/` | Generated scripts (`script_*.txt`), idea lists (`ideas_*.txt`), video plans (`plan_*.md`), and SEO packs (`seo_*.md`). A few example scripts are tracked in git. |
| `output/` | Finished videos (`output_final_*.mp4`), captions (`.ass`), thumbnails (`thumb_*.jpg`), and `latest.txt` pointing at the newest render. Safe to delete. |
